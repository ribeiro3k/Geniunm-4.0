...
export async function startChatSession(
  ...
  console.log("[startChatSession] Iniciando obtenção do prompt...");
  let systemInstructionContent = GEMINI_SIMULATOR_PROMPT_TEMPLATE; // Default
  let promptSource = "Padrão (Hardcoded)";

  if (supabase) {
    console.log(`[startChatSession] Tentando buscar prompt do Supabase (Tabela: ${TABLE_CONFIGURACOES_IA}, ID: ${GLOBAL_SIMULATOR_PROMPT_ID})...`);
    try {
        ...
        if (error) {
            if (error.code === 'PGRST116') { // PGRST116: "Searched for a single row, but 0 rows were found"
                console.log("[startChatSession] Nenhum prompt customizado encontrado no Supabase (ID não existe). Usando fallback.");
            } else {
                console.warn('[startChatSession] Erro ao buscar prompt customizado do Supabase (não PGRST116):', error.message, 'Usando fallback.');
            }
        } else if (data && data.prompt_content && data.prompt_content.trim() !== '') {
            systemInstructionContent = data.prompt_content;
            promptSource = "Supabase";
            console.log("[startChatSession] Prompt customizado carregado do Supabase com SUCESSO.");
        } else if (data && (!data.prompt_content || data.prompt_content.trim() === '')) {
            console.log("[startChatSession] Prompt customizado encontrado no Supabase, mas está VAZIO. Usando fallback.");
        } else {
             console.log("[startChatSession] Resposta do Supabase não continha dados de prompt esperados (data era null ou undefined). Usando fallback.");
        }
    } catch (e) {
        console.warn('[startChatSession] Exceção CRÍTICA ao buscar prompt customizado do Supabase:', (e as Error).message, 'Usando fallback.');
    }
  } else {
     console.warn(`[startChatSession] Supabase NÃO está configurado. ${SUPABASE_ERROR_MESSAGE}`);
     console.log("[startChatSession] Tentando buscar prompt do localStorage como fallback...");
     ...
     if (localPrompt && localPrompt.trim() !== '') {
        ...
        promptSource = "LocalStorage (Supabase Indisponível)";
        console.log("[startChatSession] Prompt customizado carregado do localStorage com SUCESSO.");
     } else {
        console.log("[startChatSession] Nenhum prompt encontrado no localStorage ou prompt vazio. Mantendo padrão.");
     }
  }
  
  console.log(`[startChatSession] Prompt FINAL que será usado pela IA (Origem: ${promptSource}):\n`, systemInstructionContent.substring(0, 300) + "...");


  let finalSystemInstruction = systemInstructionContent;
  finalSystemInstruction = finalSystemInstruction.replace(/{SCENARIO_TITLE}/g, scenario.title || "Indefinido");
  finalSystemInstruction = finalSystemInstruction.replace(/{SCENARIO_CONTEXT}/g, scenario.context || "Contexto geral de um aluno interessado em EAD.");
  finalSystemInstruction = finalSystemInstruction.replace(/{SCENARIO_INITIAL_MESSAGE_CONTEXT}/g, scenario.initialMessage || "Interesse geral em cursos EAD.");
  finalSystemInstruction = finalSystemInstruction.replace(/{BEHAVIORAL_PROFILE}/g, scenario.behavioralProfile || "Padrão");
  ...
  console.log("[startChatSession] Criando chat com a IA...");
  const currentChat = ai.chats.create({
    model: 'gemini-2.5-flash-preview-04-17',
    config: {
      systemInstruction: finalSystemInstruction,
      // Se precisar de mais configurações, adicione aqui (temperature, topK, etc.)
    },
  ...
  console.log("[startChatSession] Sessão de chat pronta. Mensagem inicial da IA (se houver):", firstAiMessage || "Nenhuma");
...
export async function evaluateObjectionResponse(objectionText: string, userResponseText: string, objectionContext?: string): Promise<string> {
  ...
  try {
    const response: GenerateContentResponse = await ai.models.generateContent({
      model: 'gemini-2.5-flash-preview-04-17',
      contents: [{role: 'user', parts: [{text: prompt}]}],
      config: {} // No specific config for now, can add temperature later if needed
    });
    return response.text;
  } catch (error) {
...
export async function generateCollaboratorAnalysis(
  ...
  try {
    const response: GenerateContentResponse = await ai.models.generateContent({
      ...
      // Consider adding temperature or other configs if needed for analysis quality
    });
    return response.text;
  } catch (error) {
